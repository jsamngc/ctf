...
import { FormInput, H4, P, FileUploader, Card, Link, Button, Modal, ModalHeader, ModalCloseButton } from "@c1ds/components"
		...
		<FormSection title="Talking Points" showDivider={isCreate}>
			<Box gridColumn={{ base: "1 / -1" }}>
				{talkingPoint ? (
					...
					<P>
						If Talking Points aren&apos;t available, <Link onClick={onDefaultModalOpen}>default Talking Points</Link>{" "}
						will be presented based on the Event Type.
					</P>
				)}
				<Modal isOpen={defaultModalOpen} onClose={onDefaultModalClose} isCentered={true} size="lg">
					<ModalHeader>
					...
					<Box mt={{ base: "-92px", sm: "-108px", md: "-144px" }} gridColumn={{ base: "1 / -1" }}>
						<P>
							We are aware of the event at hand. We are monitoring the event closely and will update you with any
							new information that comes forth during this unprecedented time. We thank you for your understanding
							and ultimately for your cooperation.
						</P>
					</Box>
					<Flex justifyContent="center" alignItems="flex-end" gridColumn={{ base: "1 / -1" }}>
							...
							<Card id={`talkingPointItem`}>
								<Flex w="full" my={{ base: "-8px", sm: "-12px" }} flexDir={{ base: "row" }}>
									<Flex flexGrow={1} justifyContent="flex-start">
										<Link href={`${talkingPoint.fileDataURL}`} download={talkingPoint.fileName}>
						...
						<Controller
							...
							render={({ onChange }) => (
								<FileUploader
									...
									allowedFileTypes={[".doc", ".docx", ".txt", ".rtf", ".pdf"]}
									onDrop={(_, files) => {
